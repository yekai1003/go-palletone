...
func (w *encbuf) toBytes() []byte {
	...
	for _, head := range w.lheads {
		...
		pos += len(enc)
		log.Debugf("head offset:%d,size:%d,out:%x", head.offset, head.size, out)
...
func makeStructWriter(typ reflect.Type) (writer, error) {
	...
	writer := func(val reflect.Value, w *encbuf) error {
		...
		for _, f := range fields {
			fvalue := val.Field(f.index)
			log.Debugf("Value:%s,Kind:%s,data:%s", fvalue.String(), fvalue.Kind().String(), fvalue.Type().String())
			if err := f.info.writer(fvalue, w); err != nil {
				return err
			}
		}
		w.listEnd(lh)
