...
func NewDefaultSecurityLevel(keyStorePath string) (bccsp.BCCSP, error) {
	...
	if err := ks.Init(nil, keyStorePath, false); err != nil {
		return nil, fmt.Errorf("Failed initializing key store at [%v]", keyStorePath)
	}
...
func New(securityLevel int, hashFamily string, keyStore bccsp.KeyStore) (bccsp.BCCSP, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed initializing configuration at [%v,%v]", securityLevel, hashFamily)
	}
...
func (csp *impl) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed generating key with opts [%v]", opts)
	}
...
func (csp *impl) KeyDeriv(k bccsp.Key, opts bccsp.KeyDerivOpts) (dk bccsp.Key, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed deriving key with opts [%v]", opts)
	}
...
func (csp *impl) KeyImport(raw interface{}, opts bccsp.KeyImportOpts) (k bccsp.Key, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed importing key with opts [%v]", opts)
	}
	...
	if !opts.Ephemeral() {
		...
		err = csp.ks.StoreKey(k)
		if err != nil {
			return nil, fmt.Errorf("Failed storing imported key with opts [%v]", opts)
		}
	}
...
func (csp *impl) Hash(msg []byte, opts bccsp.HashOpts) (digest []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed hashing with opts [%v]", opts)
	}
...
func (csp *impl) GetHash(opts bccsp.HashOpts) (h hash.Hash, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed getting hash function with opts [%v]", opts)
	}
...
func (csp *impl) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed signing with opts [%v]", opts)
	}
...
func (csp *impl) Verify(k bccsp.Key, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	...
	if err != nil {
		return false, fmt.Errorf("Failed verifing with opts [%v]", opts)
	}
